...
![Kubernetes Single Node on Minikube](/images/docs/k8s-singlenode-minikube.png)
...
**Note: These steps have not been tested with the [Minikube For Mac or Minikube For Windows beta programs](https://blog.minikube.com/2016/03/minikube-for-mac-windows-beta/).**

1. You need to have minikube installed on one machine.
...
minikube run -d \
    --volume=/:/rootfs:ro \
    --volume=/sys:/sys:rw \
    --volume=/var/lib/minikube/:/var/lib/minikube:rw \
...
```shell
minikube-machine ssh `minikube-machine active` -N -L 8080:localhost:8080
```

Setting up a ssh tunnel is applicable to remote minikube hosts as well.
...
Now run `minikube ps` you should see nginx running. You may need to wait a few minutes for the image to get pulled.
